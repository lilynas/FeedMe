...
async function generateSummary(title, content) {
  try {
    ...
    // 准备提示词
    const prompt = `
你是一个专业的且拥有丰富金融知识和理财经验的新闻简报报道员。请根据以下文章标题和内容，生成一个简洁、准确的中文摘要，不使用Markdown。
摘要需要包含以下部分应该做到以下几点：

1.对主要内容进行概括。用简洁的语言（建议2句话或50字左右）清晰概括这篇新闻报道的核心事件、基本事实。这一部分仅作总结，不要生成小标题。
2.对文内的精华信息提炼。提取并列出2个最重要的精华信息点。这些信息点应体现新闻的核心价值、关键影响或最值得关注的结论。这个部分请作为<重要信息点>，具体内容以序号的方式（1. 2. 3.）分条列举。
3.对信息的影响力进行分析。首先分析此新闻事件可能带来的1-2个主要宏观影响或潜在后果。评估此信息对中国、美国、欧洲、日本、印度股票市场（在“重大利好”、“利好”、”无影响“、“利空”、“重大利空”这五个选项中评级，如果不对中国、美国、欧洲、日本、印度股票市场造成短期影响，则不进行评级）以及国际原油价格、国际黄金市场可能产生的短期影响（在“上涨”、“下跌”、“无影响”这三个选项中评级，如果不对国际原油价格、国际黄金市场，则不进行评级），并在10字以内简单说明评级原因。这个部分请作为<影响力分析>，具体内容以序号的方式（1. 2. 3.）分条列举。
4.附加要求：务必保持客观，不添加个人观点,如果文章内容为空或不包含有效信息，不要生成文章标题或内容未提及的无关内容。对非中文的标题进行翻译，不要翻译中文标题。
